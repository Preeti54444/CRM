...
function saveCrmSessionTimerState() {
  if (!crmTimer) return
    ...
    // Also persist to backend timer metrics when API base is configured
    try {
      const session = JSON.parse(localStorage.getItem('crm_session') || '{}')
      const token = session?.access_token || session?.token || null
      // Use authoritative session id when available to avoid permission errors
      const sessionUserId = session?.id || session?.uid || session?.user_id || session?.userId || null
      if (sessionUserId) {
        if (crmTimer.userId && String(crmTimer.userId) !== String(sessionUserId)) {
          console.info('[crm-timer] overriding crmTimer.userId with session id to avoid 403', { crmTimerUserId: crmTimer.userId, sessionUserId })
        }
        crmTimer.userId = sessionUserId
      } else {
        // Prefer explicit id fields; if missing, try decoding JWT to extract subject/user id.
        crmTimer.userId = crmTimer.userId || session?.id || session?.user_id || session?.uid || session?.userId || null
        if (!crmTimer.userId && token) {
          const payload = _parseJwt(token)
          crmTimer.userId = payload?.sub || payload?.user_id || payload?.id || payload?.uid || null
        }
        // As a last resort, fall back to email (but backend expects UUID user id)
        if (!crmTimer.userId) crmTimer.userId = session?.email || ''
      }
      if (token && window.API_BASE) {
        const payload = {
          work_seconds: crmTimer.workSec || 0,
          call_seconds: crmTimer.callSec || 0,
          break_seconds: crmTimer.breakSec || 0,
          meeting_seconds: crmTimer.meetingSec || 0,
          call_count: crmTimer.callCount || 0
        }

        const doPut = (userId) => {
          if (!userId) return
          // optimistic cache write so UI can reflect values immediately even before server responds
          try {
            const cacheKey = 'crm_timer_metrics_cache'
            const existing = JSON.parse(localStorage.getItem(cacheKey) || '{}')
            const entry = {
              user_id: userId,
              work_seconds: payload.work_seconds || 0,
              call_seconds: payload.call_seconds || 0,
              break_seconds: payload.break_seconds || 0,
              meeting_seconds: payload.meeting_seconds || 0,
              call_count: payload.call_count || 0
            }
            existing[String(userId)] = entry
            try {
              const sessionSnap = JSON.parse(localStorage.getItem('crm_session') || '{}')
              if (sessionSnap && sessionSnap.email) existing[String(sessionSnap.email)] = entry
            } catch (e) { /* ignore session parse error */ }
            localStorage.setItem(cacheKey, JSON.stringify(existing))
          } catch (e) {
            // ignore cache write failures
          }
          fetch(`${window.API_BASE.replace(/\/$/, '')}/api/timer-metrics/${encodeURIComponent(userId)}`, {
            method: 'PUT',
            headers: {
            ...
            if (!res.ok) {
              res.text().then(text => {
                console.warn('[crm-timer] PUT /api/timer-metrics', res.status, res.statusText, '\nuserId=', userId, '\nsessionSnapshot=', (() => {
                  try { const s = JSON.parse(localStorage.getItem('crm_session')||'{}'); return { id: s.id||null, email: s.email||null, backendAuth: !!s.backendAuth, tokenPreview: s.access_token ? (String(s.access_token).slice(0,10) + '...' + ' len=' + String(s.access_token).length) : null } } catch(e){return {error:'parse_failed'} }
                })(), '\nresponseBody=', text)
              })
              return res
            }

            // Successful response: attempt to update in-page employee metrics so UI reflects changes immediately
            res.json().then(metric => {
              try {
                // persist a local cache of the latest metric so UI can reflect it
                try {
                  const cacheKey = 'crm_timer_metrics_cache'
                  const existing = JSON.parse(localStorage.getItem(cacheKey) || '{}')
                  const keyId = String(metric.user_id || userId)
                  existing[keyId] = metric
                  try {
                    const sessionSnap = JSON.parse(localStorage.getItem('crm_session') || '{}')
                    if (sessionSnap && sessionSnap.email) existing[String(sessionSnap.email)] = metric
                  } catch (e) { /* ignore session parse error */ }
                  localStorage.setItem(cacheKey, JSON.stringify(existing))
                } catch (e) {
                  // ignore cache write failures
                }
                const uid = String(metric.user_id || userId)
                const mgr = window.adminEmployeesManager
                if (mgr && Array.isArray(mgr.allEmployees)) {
                  // update in allEmployees
                  const empAll = mgr.allEmployees.find(e => String(e.id) === uid || String(e.email) === uid)
                  if (empAll) {
                    empAll.workSeconds = metric.work_seconds || 0
                    empAll.callSeconds = metric.call_seconds || 0
                    empAll.breakSeconds = metric.break_seconds || 0
                    empAll.meetingSeconds = metric.meeting_seconds || 0
                    empAll.callCount = metric.call_count || 0
                  }

                  // update in visible employees list if present
                  if (Array.isArray(mgr.employees)) {
                    const empVis = mgr.employees.find(e => String(e.id) === uid || String(e.email) === uid)
                    if (empVis) {
                      empVis.workSeconds = metric.work_seconds || 0
                      empVis.callSeconds = metric.call_seconds || 0
                      empVis.breakSeconds = metric.break_seconds || 0
                      empVis.meetingSeconds = metric.meeting_seconds || 0
                      empVis.callCount = metric.call_count || 0
                    }
                  }

                  // force re-render of the table
                  if (typeof mgr.renderEmployees === 'function') {
                    mgr.renderEmployees()
                  } else if (typeof mgr.filterEmployees === 'function') {
                    mgr.filterEmployees()
                  }

                  // Additionally, try to update the specific table row DOM cells
                  try {
                    const tbody = document.getElementById('employeesBody')
                    if (tbody && typeof mgr !== 'undefined') {
                      const rows = Array.from(tbody.querySelectorAll('tr'))
                        .filter(r => !r.id || !r.id.startsWith('detailsRow-'))
                      for (const row of rows) {
                        const tds = row.querySelectorAll('td')
                        if (!tds || tds.length < 12) continue
                        const rowEmail = (tds[1] && tds[1].textContent || '').trim()
                        // try to extract id from the show button onclick if present
                        let rowId = null
                        const showBtn = row.querySelector("button[onclick*='showEmployeeProfile']")
                        if (showBtn) {
                          const onclick = showBtn.getAttribute('onclick') || ''
                          const m = onclick.match(/showEmployeeProfile\('([^']+)'\)/)
                          if (m) rowId = m[1]
                        }

                        // Use the manager's in-memory employee object (preferred source of truth)
                        const emp = (mgr.employees || []).find(e => (rowId && String(e.id) === String(rowId)) || (e.email && e.email === rowEmail))
                        if (emp) {
                          try {
                            const fmt = typeof mgr.formatSeconds === 'function' ? mgr.formatSeconds.bind(mgr) : (s => formatCrmTimerTime(Number(s || 0)))
                            tds[8].textContent = fmt(emp.workSeconds || 0)
                            tds[9].textContent = `${emp.callCount || 0} / ${fmt(emp.callSeconds || 0)}`
                            tds[10].textContent = fmt(emp.breakSeconds || 0)
                            tds[11].textContent = fmt(emp.meetingSeconds || 0)
                          } catch (e) {
                ...
                console.warn('[crm-timer] failed to merge metric into adminEmployeesManager', e)
              }
            }).catch(() => {})

            return res
          }).catch(err => {
            console.error('[crm-timer] PUT /api/timer-metrics fetch error', err)
          })
        }

        // If we only have an email (contains '@') or missing id, try resolving via /auth/me
        const looksLikeEmail = String(crmTimer.userId || '').includes('@')
        if (!crmTimer.userId || looksLikeEmail) {
          fetch(`${window.API_BASE.replace(/\/$/, '')}/auth/me`, {
            method: 'GET',
            headers: { 'Authorization': `Bearer ${token}` }
          }).then(r => {
            if (!r.ok) throw new Error('failed to fetch /auth/me')
            return r.json()
          }).then(user => {
            if (user && user.id) {
              crmTimer.userId = user.id
              doPut(user.id)
            } else if (crmTimer.userId) {
              // fallback to existing value (may be email) — attempt PUT anyway
              doPut(crmTimer.userId)
            }
          }).catch(() => {
            // best-effort fallback
            if (crmTimer.userId) doPut(crmTimer.userId)
          })
        } else {
          doPut(crmTimer.userId)
        }
      ...
      <div style="padding:12px 14px;margin-bottom:12px;border:1px solid #e5e7eb;border-radius:14px;background:#fff;">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;">
          ...
          <div style="text-align:right;min-width:88px;">
            <div style="font-size:11px;color:${req.type === 'flagged' ? '#b91c1c' : '#9B2335'};font-weight:700;text-transform:uppercase;">${req.type}</div>
...
function crmTimerUpdateUserFromSession() {
  ...
  try {
    ...
    crmTimer.userId = crmTimer.userId || data.id || data.uid || data.user_id || data.userId || data.email || ''
...
function crmTimerGetSessionUserEmail() {
  ...
  try {
    const data = JSON.parse(session)
    // Prefer using crm1.html helpers when available to ensure normalized object storage
    let users = {}
    if (typeof getCrmUsersMap === 'function') {
      users = getCrmUsersMap() || {}
    } else {
      const stored = JSON.parse(localStorage.getItem('crm_users') || '[]')
      if (Array.isArray(stored)) {
        users = stored.reduce((acc, u) => {
          try {
            const e = String(u && (u.email || u.id || '')).trim().toLowerCase()
            if (e) acc[e] = u
          } catch (e) {}
          return acc
        }, {})
    ...
    if (typeof setCrmUsersMap === 'function') {
      setCrmUsersMap(users)
    } else {
      localStorage.setItem('crm_users', JSON.stringify(users))
    }
    if (Array.isArray(users)) {
      const index = users.findIndex(u => String(u.email || '').trim().toLowerCase() === key)
      if (index >= 0) {
    ...
    try {
      if (typeof loadEmployees === 'function') {
        loadEmployees()
      } else if (typeof renderEmployeeOverview === 'function') {
        renderEmployeeOverview()
      }
    } catch (eventError) {
      // Non-fatal: only useful for refreshing employee display
      console.warn('crm-timer: unable to refresh employee display after persisting metrics', eventError)
    }
  } catch (error) {
