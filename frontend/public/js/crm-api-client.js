...
class APIClient {
  ...
  async getContacts(params = {}) {
    const query = new URLSearchParams(params).toString()
    const response = await this.get(`/contacts${query ? `?${query}` : ''}`)
    if (Array.isArray(response)) {
      return response
    }
  ...
  async checkLeadDuplicates(leadData) {
    return this.post('/leads/check-duplicates', leadData)
  }

  async getLeadPermissions(leadId) {
    return this.get(`/leads/${leadId}/permissions`)
  }

  async transferLeadOwnership(leadId, newOwnerId, transferReason) {
    return this.post(`/leads/${leadId}/transfer-ownership`, {
      new_owner_id: newOwnerId,
      transfer_reason: transferReason
    })
  }

  async getLeadOwnershipHistory(leadId) {
    return this.get(`/leads/${leadId}/ownership-history`)
  }
