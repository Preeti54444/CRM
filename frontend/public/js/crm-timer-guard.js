...
function crmTimerGuardNormalizeRole(role) {
...
function crmTimerGuardIsAdmin() {
  ...
  try {
    ...
    return crmTimerGuardNormalizeRole(currentUser.role) === 'admin'
  } catch (error) {
    return false
  }
}
...
async function crmTimerGuardCheckPerformanceLogout() {
  ...
  const path = String(session.role || '').toLowerCase() === 'employee'
    ? '/targets/logout-check'
    : '/performance/logout-check'
  const response = await crmTimerGuardFetchJson(path, {
    method: 'POST',
...
async function crmTimerGuardLogout(event) {
  ...
  try {
    ...
    if (sessionStr) {
      ...
      if (session && crmTimerGuardNormalizeRole(session.role) === 'admin') {
