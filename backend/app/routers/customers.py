...
from ..services.customer_service import create_customer_profile, get_customer_by_id, get_customer_with_lead_details, update_customer, delete_customer_profile, list_customers, list_customers_with_lead_details
...
def list_customers_endpoint(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    return list_customers_with_lead_details(db)


@router.get("/{customer_id}")
def get_customer(customer_id: UUID, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    obj = get_customer_with_lead_details(db, customer_id)
    if not obj:
