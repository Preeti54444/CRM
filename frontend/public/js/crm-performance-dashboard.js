...
async function loadPerformanceDashboard(weekStart, employeeFilter) {
  try {
    ...
    window._perfChart = new Chart(ctx, {
      type: 'line',
      data: { labels, datasets: [{ label: 'Performance Score', data: scores, backgroundColor: bgColors.map(c => c.replace('0.8', '0.2')), borderColor: bgColors.map(c => c.replace('0.8', '1')), borderWidth: 2, fill: true, tension: 0.3, pointRadius: 4 }] },
      options: {
        responsive: true,
  ...
  async function showEmployeeDrilldown(employeeId, weekStart) {
    try {
      ...
      window._perfModalChart = new Chart(ctx, { type: 'line', data: { labels, datasets: [{ label: 'Achievement %', data: values, borderColor: '#9B2335', backgroundColor: 'rgba(155,35,53,0.12)', tension: 0.2 }] }, options: { responsive: true } })
