...
function nav(btn) {
  ...
  // Support legacy forecast key by mapping it to the inline CRM section.
  const sectionKey = key === 'forecast' ? 'forecasting' : key
  ...
  if (key === 'lead-form' && typeof initializeLeadForm === 'function') {
    initializeLeadForm()
  }

  // Pre-fill lead form if navigating from call details
  if (key === 'lead-form' && typeof preFillLeadFormFromCallDetails === 'function') {
    setTimeout(() => preFillLeadFormFromCallDetails(), 100)
  }
...
function renderSection(key) {
  const renderers = {
    ...
    'logout-approvals': 'renderApprovalList',
