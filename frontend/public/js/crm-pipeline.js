...
const PipelineStore = {
  ...
  init() {
    this.state.leads = (this.load(this.leadKey) || []).map((lead) => {
      const normalizedLead = this.normalizeLead(lead)
      return {
        ...normalizedLead,
        status: this.normalizePipelineStatus(normalizedLead.status)
      }
    })
  ...
  normalizePipelineStatus(status) {
    const raw = String(status || '').trim()
    if (!raw) return ''
    const normalizedRaw = raw.toLowerCase()
    const exactMatch = PipelineConfig?.stages?.find(stage => String(stage).trim().toLowerCase() === normalizedRaw)
    ...
    if (statusAliases[normalizedRaw]) {
      return statusAliases[normalizedRaw]
    }

    if (normalizedRaw === 'new lead') return 'New Lead'
    return raw
  },

  normalizeJourneyLead(entry) {
    if (!entry || typeof entry !== 'object') return null
    const status = this.normalizePipelineStatus(entry.currentStatus || entry.status || entry.lead_status || entry.purpose || entry.ldPurpose || 'Fresh Lead') || 'New Lead'
  ...
  normalizeLead(lead) {
    return {
      ...lead,
      companyName: lead.companyName || lead.company || lead.customerCompany || lead.customerName || lead.company_name || lead.customer_name || '',
      name: lead.name || lead.contactPerson || lead.leadName || lead.contact_name || lead.contactName || 'Unknown Lead',
  ...
  getStageLeads(stage) {
    return this.applyFilters(this.state.leads.filter(lead => this.normalizePipelineStatus(lead.status) === stage))
  },

  getPipelineMetrics() {
    const metrics = {}
    this.state.leads.forEach(lead => {
      const stage = this.normalizePipelineStatus(lead.status)
      if (!stage) return
  ...
  applyFilters(leads) {
    return leads.filter(lead => {
      ...
      const normalizedStatus = this.normalizePipelineStatus(lead.status)
  ...
  isSlaBreached(lead) {
    const status = this.normalizePipelineStatus(lead.status)
    const rule = PipelineConfig.slaRules[status]
  ...
  async updateLeadStatus(leadId, newStatus, details = {}) {
    ...
    try {
      const normalized = this.normalizePipelineStatus(newStatus)
      if (normalized === 'Login with Lender' || normalized === 'Login Lender' || normalized === 'Lender Selected') {
        if (typeof window.DataStore?.syncLeadToDeal === 'function') {
          window.DataStore.syncLeadToDeal(lead)
        }
        window.currentLeadId = lead.id
        if (typeof openLenderRequirementsModal === 'function') {
          setTimeout(() => openLenderRequirementsModal(lead.id), 300)
        }
      }
    } catch (e) {
      console.warn('Failed to trigger lender modal', e)
    }
...
const PipelineUI = {
  ...
  renderCard(lead) {
    ...
    const hasCompany = Boolean(lead.companyName)
    return `
      <article class="pipeline-card" draggable="true" data-lead-id="${lead.id}" ondragstart="PipelineUI.handleDragStart(event, '${lead.id}')">
        <div class="card-row"><div class="card-title">${hasCompany ? lead.companyName : lead.name}</div><div class="value-chip">₹${(lead.loanAmount / 100000).toFixed(1)}L</div></div>
        ${hasCompany ? `<div class="card-subtitle">${lead.name}</div>` : ''}
