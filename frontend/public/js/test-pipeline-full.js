...
function loadPipelineScript() {
  const context = {
    ...
    RegExp,
    Error,
    fetch: () => Promise.reject(new Error('fetch not implemented'))
  };
  vm.createContext(context);
  vm.runInContext(fs.readFileSync('js/crm-pipeline.js', 'utf8'), context);
  return context;
}
...
await test('Pipeline card render includes company name and contact person', () => {
  const ctx = loadPipelineScript();
  assert(ctx.PipelineStore, 'PipelineStore not loaded');
  assert(ctx.PipelineUI, 'PipelineUI not loaded');

  const normalized = ctx.PipelineStore.normalizeLead({
    company: 'Vendor Finance',
    contactPerson: 'preeti',
    loanType: 'Lead Journey',
    city: 'Unknown',
    mobile: '1234567890',
    loanAmount: 0,
    leadScore: 87,
    assignedEmployee: 'Saleem Khan',
    status: 'New Lead',
    lastActivity: new Date().toISOString(),
    stageEnteredAt: new Date().toISOString()
  });

  assert(normalized.companyName === 'Vendor Finance', 'companyName normalization failed');
  assert(normalized.name === 'preeti', 'name normalization failed');

  const cardHtml = ctx.PipelineUI.renderCard(normalized);
  assert(cardHtml.includes('Vendor Finance'), 'Company name missing in rendered card');
  assert(cardHtml.includes('preeti'), 'Contact person missing in rendered card');
  assert(cardHtml.includes('Lead Journey'), 'Loan type missing in rendered card');
});
