...
def create_sod(
    ...
    report_response = _report_response(report, "sod")
    background_tasks.add_task(
...
def create_eod(
    ...
    # Broadcast data sync event for real-time updates
    report_response = _report_response(report, "eod")
    background_tasks.add_task(
        broadcast_data_sync_sync,
        'eod',
        'create',
...
def create_wod(
    ...
    # Broadcast data sync event for real-time updates
    report_response = _report_response(report, "wod")
    background_tasks.add_task(
        broadcast_data_sync_sync,
        'wod',
        'create',
...
def get_weekly_performance(week_start: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    reports = list_wod_reports_service(db, current_user=current_user, admin=(str(getattr(current_user, 'role', '')).lower() == "admin"))
    filtered_reports = []
    for report in reports:
        if hasattr(report, 'week_start') and report.week_start:
            try:
                from datetime import datetime
                report_date = report.week_start if isinstance(report.week_start, datetime) else datetime.fromisoformat(str(report.week_start))
                if report_date.strftime('%Y-%m-%d') == week_start:
                    filtered_reports.append(_report_response(report, "wod"))
            except:
                pass
