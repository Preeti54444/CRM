...
api_router = APIRouter(prefix="/api/timer", tags=["timer"])
...
@api_router.post("/early-logout/request", response_model=EarlyLogoutRequestResponse, status_code=status.HTTP_201_CREATED)
def api_request_early_logout(
    payload: EarlyLogoutRequestCreate,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    return request_early_logout(payload, db=db, current_user=current_user)


@api_router.get("/early-logout/pending", response_model=EarlyLogoutRequestResponse)
def api_get_pending_early_logout_request(
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    return get_pending_early_logout_request(db=db, current_user=current_user)


@api_router.get("/early-logout/requests", response_model=List[EarlyLogoutRequestResponse])
def api_list_early_logout_requests_route(
    db: Session = Depends(get_db),
    current_user=Depends(require_manager_or_admin),
):
    return list_early_logout_requests_route(db=db, current_user=current_user)


@api_router.post("/early-logout/review", response_model=EarlyLogoutRequestResponse)
def api_review_early_logout_request_route(
    payload: EarlyLogoutRequestReview,
    db: Session = Depends(get_db),
    current_user=Depends(require_manager_or_admin),
):
    return review_early_logout_request_route(payload, db=db, current_user=current_user)
