...
def get_admin_notification_recipients(db: Session, exclude_user_id: UUID | str | None = None) -> list[User]:
    admin_role_values = {"admin", "manager"}
    query = db.query(User).filter(
        func.lower(User.role).in_(list(admin_role_values)),
        User.status == "active",
    )
    if exclude_user_id is not None:
        try:
            exclude_uuid = UUID(str(exclude_user_id))
