...
const FollowUpReminders = {
  ...
  addSidebarMenuItem() {
    ...
    if (!navMenu) return;
    if (navMenu.querySelector('[data-sec="followup-reminders"]')) return;
    
    const reminderBtn = document.createElement('div');
    reminderBtn.className = 'nav-grp';
    // Create accessible button element instead of raw innerHTML
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'nav-btn';
    btn.dataset.sec = 'followup-reminders';
    btn.setAttribute('aria-label', 'Follow-up Reminders');
    ...
    btn.addEventListener('click', function (e) { FollowUpReminders.showRemindersPanel(this); });
    btn.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        FollowUpReminders.showRemindersPanel(this);
      }
    });

    reminderBtn.appendChild(btn);
    ...
    if (dashboardBtn) {
      ...
      navMenu.appendChild(reminderBtn);
    }
  },
  
  // Show reminders panel
  showRemindersPanel(btn) {
    const contentContainer = document.querySelector('.content');
    if (!contentContainer) {
      console.warn('[FollowUp Reminders] Content container not found');
      return;
    }

    // Ensure the section exists before navigation takes place
    let remindersSection = document.getElementById('sec-followup-reminders');
    if (!remindersSection) {
      remindersSection = this.createRemindersSection();
      contentContainer.appendChild(remindersSection);
    }

    if (btn && typeof nav === 'function') {
      nav(btn);
      return;
    }
...
// Renderer hook used by crm-navigation nav() for follow-up reminders
function renderFollowupReminders() {
  // Ensure there's always a section element for nav() to activate to avoid a blank page
  const contentContainer = document.querySelector('.content');
  if (contentContainer && !document.getElementById('sec-followup-reminders')) {
    // Create a lightweight placeholder section so nav() has something to show immediately
    const placeholder = document.createElement('section');
    placeholder.className = 'section';
    placeholder.id = 'sec-followup-reminders';
    placeholder.innerHTML = `<div style="padding:40px;text-align:center;color:var(--gray-500);">Loading follow-ups...</div>`;
    contentContainer.appendChild(placeholder);
  }

  // If the FollowUpReminders object isn't ready yet, poll briefly and then replace placeholder
  if (!window.FollowUpReminders) {
    console.warn('[FollowUp Reminders] renderFollowupReminders waiting for FollowUpReminders to initialize');
    let attempts = 0;
    const waitForInit = () => {
      if (window.FollowUpReminders) {
        const existing = document.getElementById('sec-followup-reminders');
        if (existing) {
          existing.replaceWith(window.FollowUpReminders.createRemindersSection());
        } else if (contentContainer) {
          contentContainer.appendChild(window.FollowUpReminders.createRemindersSection());
        }
        window.FollowUpReminders.loadAndDisplayReminders();
        return;
      }
      attempts += 1;
      if (attempts >= 20) {
        console.warn('[FollowUp Reminders] FollowUpReminders did not initialize in time');
        return;
      }
  ...
  // Normal path: FollowUpReminders is available
  if (contentContainer && !document.getElementById('sec-followup-reminders')) {
    contentContainer.appendChild(window.FollowUpReminders.createRemindersSection());
  }
